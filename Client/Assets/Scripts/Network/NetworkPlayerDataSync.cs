...
public class NetworkPlayerDataSync : NetworkBehaviour
{
    ...
    public NetworkVariable<string> networkElementType = new NetworkVariable<string>("Fire");
    public NetworkVariable<string> networkGender = new NetworkVariable<string>("Male");
    public NetworkVariable<string> networkCharacterName = new NetworkVariable<string>("");
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsServer)
        {
            // Server: Load player data từ GameManager và set vào NetworkVariable
    ...
    private void LoadPlayerDataFromGameManager()
    {
        ...
        {
            Debug.LogWarning("[NetworkPlayerDataSync] GameManager.Instance is null or no player data! Using default values.");
            return;
        }
        ...
        if (playerData == null)
        {
            Debug.LogWarning("[NetworkPlayerDataSync] PlayerDataResponse is null! Using default values.");
            return;
        }

        // Set NetworkVariable (chỉ server mới có quyền write)
        networkPlayerId.Value = playerData.player_id;
        networkElementType.Value = playerData.element_type ?? "Fire";
        networkGender.Value = playerData.gender ?? "Male";
        networkCharacterName.Value = playerData.character_name ?? "";
    ...
    private void ApplyPlayerData()
    {
        ...
        // Có thể cần thay đổi SpriteRenderer hoặc Animator dựa trên element_type + gender
        ApplyVisuals();
    }
    ...
    private void ApplyVisuals()
    {
        ...
        // Ví dụ: Load sprite từ Resources hoặc thay đổi Animator Controller
    ...
    private void OnElementTypeChanged(string oldValue, string newValue)
    {
        Debug.Log($"[NetworkPlayerDataSync] Element type changed: {oldValue} → {newValue}");
        ApplyVisuals();
    }

    private void OnGenderChanged(string oldValue, string newValue)
    {
        ...
        int level, int hp, int maxHp, int mp, int maxMp, int attack, float moveSpeed)
    {
        ...
        networkElementType.Value = elementType;
        networkGender.Value = gender;
        networkCharacterName.Value = characterName;
    ...
    /// </summary>
    public string GetCharacterName() => networkCharacterName.Value;
    public string GetElementType() => networkElementType.Value;
    public string GetGender() => networkGender.Value;
