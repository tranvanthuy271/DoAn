...
public class ItemTemplateManager : MonoBehaviour
{
    ...
    public static void EnsureInstance()
    {
        ...
        { /* EnsureInstance: tự tạo singleton cho dedicated server */ }
    }
    ...
    void Awake()
    {
        ...
        if (Instance == null)
        {
            ...
            { /* Singleton initialized - GameObject: {gameObject.name} */ }
        }
        else
        {
            { /* Cảnh báo: ⚠️ Duplicate instance detected! Destroying {gameObject.name} */ }
            Destroy(gameObject);
            return;
        }
    }
    ...
    void Start()
    {
        { /* 🚀 Start() called - autoLoadOnStart={autoLoadOnStart} */ }
        if (autoLoadOnStart)
        {
        ...
        {
            { /* AutoLoad is disabled, call LoadItemTemplatesFromAPI() manually */ }
        }
    }
    ...
    private System.Collections.IEnumerator LoadItemTemplatesWhenReady()
    {
        ...
        {
            { /* Cảnh báo: APIClient null (dedicated server?) → dùng UnityWebRequest trực tiếp */ }
            StartCoroutine(LoadItemTemplatesDirect());
        }
        else
        {
            { /* APIClient đã sẵn sàng sau {elapsed:F1}s */ }
    ...
    private System.Collections.IEnumerator LoadItemTemplatesDirect()
    {
        ...
        { /* LoadItemTemplatesDirect: GET {url} */ }
        ...
        using (var www = UnityWebRequest.Get(url))
        {
            ...
            {
                { /* Lỗi: LoadItemTemplatesDirect FAILED: {www.error} */ }
                isLoading = false;
                yield break;
            }
            ...
            try
            {
                ...
                if (response != null && response.item_templates != null)
                {
                    ...
                    isLoaded = true;
                    { /* LoadItemTemplatesDirect: loaded {response.item_templates.Length} templates */ }
                }
                else
                {
                    { /* Lỗi: LoadItemTemplatesDirect: response hoặc item_templates null */ }
                }
            }
            catch (System.Exception ex)
            {
                { /* Lỗi: LoadItemTemplatesDirect parse error: {ex.Message} */ }
            }
            isLoading = false;
        }
    }
    ...
    {
        { /* LoadItemTemplatesFromAPI() called - isLoading={isLoading}, isLoaded={isLoaded} */ }
        
        if (isLoading)
        {
            { /* Cảnh báo: ⏳ Đang loading item templates, vui lòng đợi */ }
            return;
        }

        if (isLoaded)
        {
            { /* Item templates đã được load trước đó ({itemTemplatesById.Count} items) */ }
            return;
        }
    ...
    {
        { /* 📦 OnItemTemplatesLoaded() - Received {templates.Length} templates */ }
        ...
        { /* Đã load {templates.Length} item templates thành công */ }
        { /* Dictionary Stats - ById: {itemTemplatesById.Count}, ByCode: {itemTemplatesByCode.Count} */ }
        
        // Log top 10 items để debug
        int logCount = Mathf.Min(10, templates.Length);
        { /* Logging first {logCount} items */ }
        for (int i = 0; i < logCount; i++)
        {
            var template = templates[i];
            { /* ID={template.id}, Name='{template.name}', Code='{template.code}', IconId='{template.icon_id}', Type={template.item_type}, Stackable={template.stackable} */ }
        }
    }
    ...
    public ItemTemplateDto GetItemTemplate(int id)
    {
        if (!isLoaded)
        {
            { /* Cảnh báo: ⚠️ Item templates chưa được load! Gọi LoadItemTemplatesFromAPI() trước */ }
            return null;
        }
        ...
        if (itemTemplatesById.TryGetValue(id, out var template))
        {
            if (enableDebugLog)
            {
                { /* Found item template ID={id}: '{template.name}' (code={template.code}) */ }
            }
            return template;
        }

        { /* Cảnh báo: Item template với ID {id} không tồn tại! Total loaded: {itemTemplatesById.Count} */ }
        return null;
    }
    ...
    public ItemTemplateDto GetItemTemplateByCode(string code)
    {
        if (!isLoaded)
        {
            { /* Cảnh báo: Item templates chưa được load! Gọi LoadItemTemplatesFromAPI() trước */ }
            return null;
        }
        ...
        { /* Cảnh báo: Item template với code '{code}' không tồn tại */ }
        return null;
    }
    ...
    public ItemTemplateDto[] GetAllItemTemplates()
    {
        if (!isLoaded)
        {
            { /* Cảnh báo: Item templates chưa được load */ }
