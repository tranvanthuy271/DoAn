...
public class EnemySkillSet : MonoBehaviour
{
    ...
    // Nguyên tố của enemy (Fire/Water/None/…). Dùng để hiển thị UI.
    public string ElementType { get; private set; } = "None";

    // Danh sách skill đã được load và validate. Readonly sau SetSkillsFromConfig().
    public IReadOnlyList<SkillEntry> Skills => _skills;

    // True khi đã gọi SetSkillsFromConfig() ít nhất một lần.
    ...
    // Tham số distToTarget: Khoảng cách thực (Unity units) đến mục tiêu gần nhất.
